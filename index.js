...
const MONGOURL = process.env.MONGODB_URI || "mongodb+srv://jyothikakj:gS0YqCfHRYsKGO0j@cluster0.vxfvo.mongodb.net/BankDB?retryWrites=true&w=majority&appName=Cluster0";
...
app.post("/register", async (req, res) => {
    try {
        ...
        const hashedPassword = await bcrypt.hash(password, 10);//10 is the higher saltRounds value increases security but also makes the hashing process slower.
...
app.post("/login", async (req, res) => {
    try {
        ...
        const token = jwt.sign({ userId: user.userId }, process.env.JWT_SECRET, { expiresIn: '1h' });//1hr kazhinja veendum login cheyyanam
...
// New route: Get user's transaction history
